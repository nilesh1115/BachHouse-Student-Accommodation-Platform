        ...
        <div className="absolute bottom-2 left-0 right-0 flex justify-center gap-1">
          {images.map((_, index) => (
            <button
              ...
              className={`h-1.5 w-1.5 sm:h-2 sm:w-2 rounded-full transition-all ${index === currentIndex ? 'bg-gray-800 w-3 sm:w-4' : 'bg-white/50 hover:bg-white/80'
                }`}
              suppressHydrationWarning
  ...
  useEffect(() => {
    const fetchFeaturedProperties = async () => {
      try {
        ...
        // Get 3 random properties from the available ones
        const shuffled = allProperties.sort(() => 0.5 - Math.random());
        const selectedProperties = shuffled.slice(0, 3).map(property => ({
          id: property._id,
          images: Array.isArray(property.image) && property.image.length > 0
            ? property.image
            : [assets.property1_image], // fallback to default image
          type: property.type || property.proptype || "Property",
          price: `₹${(property.rent || property.price || 0).toLocaleString()}/month`,
          title: property.title || property.name || "Untitled Property",
          distance: property.distance || property.location || "Location not specified",
          gender: property.gender || "Any",
          amenities: Array.isArray(property.amenities)
            ? property.amenities.slice(0, 3).concat(property.amenities.length > 3 ? [`+${property.amenities.length - 3}`] : [])
            : ["WiFi", "Furnished", "+2"]
        }));
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {loading ? (
          <div className="flex justify-center items-center py-12">
            ...
            <span className="ml-3 text-gray-600">Loading featured properties...</span>
          </div>
        ) : (
              ...
              >
                <div className="p-1 sm:p-2 relative">
                  <div className="relative rounded-md sm:rounded-lg overflow-hidden">
                    ...
                    <button
                      ...
                      className={`absolute top-12 right-3 sm:top-14 sm:right-4 p-1.5 rounded-full shadow-md z-10 transition-all ${favorites[property.id]
                    ...
                    >
                      <Heart
                        className={`h-3.5 w-3.5 sm:h-4 sm:w-4 ${favorites[property.id] ? 'fill-current' : ''
                          }`}
                      />
                    </button>
                  </div>
                ...
                <div className="p-4 sm:p-5 md:p-6 flex flex-col flex-grow">
                  ...
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 sm:gap-0 mb-3 sm:mb-4">
                    ...
                    <div className="flex">
                      <div className={`inline-flex items-center px-2 py-0.5 sm:px-3 sm:py-1 rounded-full text-xs sm:text-sm font-medium transition-colors ${property.gender === "Male Only" ? "bg-blue-100 text-blue-800 hover:bg-blue-200" :
